...
# sudo -s
# cd /tmp
# wget -O collation.sh https://raw.githubusercontent.com/deforay/utility-scripts/refs/heads/master/lamp/fix-db-collation.sh
# chmod +x ./collation.sh
# ./collation.sh
...
check_index_warnings() {
    ...
    if [[ -n "$indexes_info" ]]; then
        ...
        while IFS=$'\t' read -r index_name non_unique index_type; do
...
# Function to build column definition
...
# Function to verify column conversion
verify_column_conversion() {
    local table_name=$1
    local column_name=$2

    local current_collation
    current_collation=$(mysql -h"$MYSQL_HOST" -P"$MYSQL_PORT" -u"$MYSQL_USER" -p"$MYSQL_PASSWORD" \
        -e "SELECT COLLATION_NAME FROM information_schema.columns
            WHERE table_schema = '$DATABASE_NAME'
            AND table_name = '$table_name'
            AND column_name = '$column_name';" -s -N)
...
process_tables() {
    ...
    for table_info in "${table_array[@]}"; do
        ...
        if [[ -n "$columns_info" ]]; then
            ...
            while IFS=$'\t' read -r column_name column_type is_nullable column_default extra collation_name column_comment; do
                ((column_count++))
            done <<< "$columns_info"
